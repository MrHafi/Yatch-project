...
function yacht_parse_float($value, $remove = []) {

    $value = str_replace($remove, '', (string) $value);
    $value = trim($value);
...
function yacht_parse_int($value) {
...
function yacht_get_clean_yacht_data($yacht) {
    ...
    return [
       ...
       'length_feet'     => yacht_parse_float($yacht['sizeFeet'] ?? '', [' Ft']),
'length_meter'    => yacht_parse_float($yacht['sizeMeter'] ?? '', [' m', 'm']),
'beam'            => yacht_parse_float($yacht['yachtBeam'] ?? ''),
'draft'           => yacht_parse_float($yacht['yachtDraft'] ?? ''),

'pax'             => yacht_parse_int($yacht['yachtPax'] ?? ''),
'cabins'          => yacht_parse_int($yacht['yachtCabins'] ?? ''),

'year_built'      => yacht_parse_int($yacht['yachtYearBuilt'] ?? ''),
'refit_year'      => yacht_parse_int($yacht['yachtRefit'] ?? ''),
        'builder'         => sanitize_text_field($yacht['yachtBuilder'] ?? ''),

       'low_price'       => yacht_parse_float($yacht['yachtLowNumericPrice'] ?? ''),
'high_price'      => yacht_parse_float($yacht['yachtHighNumericPrice'] ?? ''),
...
function yacht_save_yacht_row($code, $data, $details_table, $sync_time, $existing_row) {
    ...
    $db_data = [
       ...
       if ($existing_row) {
        ...
        $result = $wpdb->insert($details_table, $db_data);
    }

    if ($result === false) {
        yacht_sync_log('DB error for yacht ' . $code . ' - ' . $wpdb->last_error);
        return false;
    }
...
function yacht_process_single_yacht($code, $details_table, $sync_time) {
    ...
    $saved = yacht_save_yacht_row($code, $data, $details_table, $sync_time, $existing_row);
